...
void	display_token(void *content)
{
	...
	ft_printf("type		: %s%s%s\n", YELLOW, tmp->type == PIPE ? "PIPE" : tmp->type == FFILE ? "FFLIE" : tmp->type == REDIR_IN ? "REDIR_IN" : tmp->type == REDIR_OUT ? "REDIR_OUT" : tmp->type == APPEND ? "APPEND" : tmp->type == CMD ? "CMD" : tmp->type == HEREDOC ? "HEREDOC" : "ARG", RESET);
